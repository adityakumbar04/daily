...
function verifyAdminTokenFromReq(req) {
...
function requireAdmin(req, res, next) {
  ...
  // Back-compat: allow password query param (old admin.js) if present
  const adminPassword = req.query.password;
  if (adminPassword && safeEqual(adminPassword, process.env.ADMIN_PASSWORD || '')) {
    req.admin = { role: 'admin', username: process.env.ADMIN_USERNAME || 'admin' };
    return next();
  }
...
// Date-wise report: Q1..Q18 Yes/Total per day
app.get('/api/report', requireAdmin, async (req, res) => {
  try {
    ...
    const hasStart = typeof startParam === 'string' && startParam.trim();
    const hasEnd = typeof endParam === 'string' && endParam.trim();
