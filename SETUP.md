...
```
GET /api/responses?password=your_admin_password
```
...
#### Get Statistics (GET)
```
GET /api/stats?password=your_admin_password
```
...
- ✅ Admin password is required for all data retrieval endpoints
- ✅ Password is sent as URL parameter in admin (consider upgrading to POST/Bearer token for production)
